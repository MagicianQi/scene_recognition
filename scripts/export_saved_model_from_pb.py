...
pb_file = "./models/vgg_places365.pb"
...
with graph.as_default():
    ...
    input_bytes = tf.placeholder(tf.string, shape=[None], name='b64_images')
    decoded_image_batch = tf.map_fn(tf.image.decode_jpeg, input_bytes, dtype=tf.uint8)
    decoded_image_batch = tf.cast(decoded_image_batch, tf.float32)
    ...
    print(decoded_image_batch)
    input_map = {}
    input_map.setdefault("input_1:0", decoded_image_batch)
    ...
    embeddings = graph.get_tensor_by_name("import/predictions/Softmax:0")
